...
exports.generateTablePDF = async ({ 
  title, 
  columns, 
  data, 
  ...
  // Standard A4: [595.28, 841.89]
  const pageSize = isLandscape ? [841.89, 595.28] : [595.28, 841.89]; 
  ...
  // --- Helper: Add New Page & Draw Headers ---
  const addNewPage = () => {
    ...
    page.drawImage(logoImage, {
      x: pageMargin + (usableWidth - logoDims.width) / 2,
      y: height - pageMargin - logoDims.height,
    ...
    columns.forEach(col => {
      ...
      col.currentX = currentX; 
      currentX += col.width;
    });

    return { page, currentY: headerY - rowHeight };
  };
