...
exports.exportActiveMotorsToPDF = async (req, res) => {
  try {
    ...
    for (let motor of activeMotors) {
      motor = {
        tonNumber: motor.eq.tonNumber || 'N/A',
        designation: motor.eq.designation || 'N/A',
    ...
    for (let i = 0; i < totalPages; i++) {
        ...
        if (i === 0) {
            footerText = `Total Active Motors: ${activeMotors.length} | ${footerText}`;
        }
...
exports.exportSpareMotorsToPDF = async (req, res) => {
  try {
    ...
    for (let i = 0; i < totalPages; i++) {
        ...
        let footerText = `Page ${i + 1} of ${totalPages}`;
        if (i === 0) {
            footerText = `Total Spare Motors: ${spareMotors.length} | ${footerText}`;
        }
    ...
    res.setHeader(
      'Content-Disposition',
      `attachment; filename=spare_motors_${new Date().toISOString().split('T')[0]}.pdf`
    );
